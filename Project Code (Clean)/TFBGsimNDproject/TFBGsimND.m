...
Dn_AC = 0.0003           % index modulation amplitude of the grating
...
Npoints=1+round((WavEnd-WavStart)/0.00002)   %For spectrum points every 20 pm
...
MaskPer = BraggOnly(SpectrumName,BraggWav,RL,resol,material,x);
end
...
Neffcore = CoreOnly(SpectrumName,CalcWav,RL,resol,material,x);
end 
...
dummy0 = Modes(SpectrumName,CalcWav,Mmax,Neig,Neffmax)
end
...
dummy2 = Spectrum(Neffcore,MaskPer,Length,Dn_AC,SpectrumName,InputPol,Npoints,WavStart,WavEnd,maxdetuning)
