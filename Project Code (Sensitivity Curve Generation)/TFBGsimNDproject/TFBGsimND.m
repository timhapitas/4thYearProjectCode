...
troughs = cell(1, indexSteps);
wavelengths = cell(1, indexSteps);

for count = 1:indexSteps
...
WavEnd = 1.595 %End wavelength

Npoints=1+round((WavEnd-WavStart)/0.000001)   %For spectrum points every 20 pm
...
[NeffDiff, NeffCladCutoffIndex] = min(abs(Neff - currentIndex));
...
currentIndex = currentIndex + indexStepSize;
