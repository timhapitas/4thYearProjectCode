function Neffcorecalc = CoreOnly(SpectrumName,Wavelength,RL,resol,material,x)
% This program calculates the core mode index and fields at the calculation
% wavelength
%%
disp('calculating the core mode effective index at the median calculation wavelength ')
global nL r_nf nf
global nLC  
...
for i = 1:N_jump
    ...
    for i = 1:N_layer
        ...
        Dr{i} = Dr{i}./mat_norme;
        Er{i} = Dr{i}./(nL(i)^2);      % these are the final field with N_layer cell elements each containing a matrix with Neig columns and a number of lines equal to the number of grid points       
    ...
    CoreIndex=1;    %there is only one mode found
    
    %saving the fields of the core mode in the core region only
        E1r = Er{1}(:,CoreIndex);
        E1p = Ephi{1}(:,CoreIndex);
        E1z = Ez{1}(:,CoreIndex);
    save([SpectrumName,'/E1r'],'E1r');
    save([SpectrumName,'/E1p'],'E1p');
    save([SpectrumName,'/E1z'],'E1z');
...
%====================================================
Neffcorecalc = real(Neff(CoreIndex))
end
