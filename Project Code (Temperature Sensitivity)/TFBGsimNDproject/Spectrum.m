function [dummy, trueWavelengths, trueTroughs] = Spectrum(Neffcore,Mask_Per,Length,Dn_AC,SpectrumName,InputPol,Npoints,WavStart,WavEnd,maxdetuning,cutoffWavelength)
%{
...
DWav = vpa((WavEnd-WavStart)/(Npoints-1));
DWav = double(DWav);
...
%Now we select which modes will participate in the RK integration by checking phase matching.    
    ...
    Detuning = vpa(abs((real(Neffcore+Npm))-2*wavLoc/MaskPer));
    Detuning = double(Detuning);                  %is a matrix containing the detunings of all modes at that wavelength
    ...
    E_init(1,1)=1;                          %gives an amplitude of 1.0 for the core mode at the grating output
     ...
     [z,E0] = ode45(@f_MEcalc_P,zspan,E_init); % Now get the transmitted amplitude (absolute value) for P input
     transP(Iwav,1)= 1/(abs(E0(3,1)^2));       %The first column of matrix E0 has the amplitude squared of the forward core mode at each calculated z position                                      %The last line corresponds to the end of the integrationrange i.e. the input of the
    ...
    for Iwav = 1:length(Wavelengths)
        if Troughs(Iwav) <= -3.0
            
            trueTroughs(length(trueTroughs) + 1) = Troughs(Iwav);
            trueWavelengths(length(trueWavelengths) + 1) = Wavelengths(Iwav);
            
        end
    end
    
    initLength = length(trueWavelengths);
    trueWavelengths(trueWavelengths <= cutoffWavelength) = [];
    numIndicesToDelete = abs(initLength - length(trueWavelengths));
    trueTroughs(1:numIndicesToDelete) = [];
    ...
    plot(wav,TdBP,'b');
    hold on;
    plot(trueWavelengths, trueTroughs, 'r.');
    hold on;
    plot([cutoffWavelength cutoffWavelength], [0 min(TdBP)]);
    hold off;
    grid on;
    title('Transmission Spectrum of TFBG (Bragg Wavelength of 1610 nm, 10 Degree Tilt Angle)');
